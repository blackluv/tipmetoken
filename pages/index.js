...
import abi from "../utils/Tips.json";

export default function Home() {
  /**
   * Create a variable here that holds the contract address after you deploy!
   */
  const contractAddress = "0x6541ec5bdcb5149f36f464c333610d5e6ffc37e4";
  ...
  const buyCoffee = async () => {
    try {
      ...
      if (ethereum) {
        ...
        let count = await coffeePortalContract.getTotalTips();
        console.log("Retrieved total tips count...", count.toNumber());

        /*
         * Execute the actual coffee gift from your smart contract
         */
        const cost = 0.001;
        const coffeeTxn = await coffeePortalContract.MakeTip(
        ...
        toast.info("Sending Fund for tip...", {
          position: "top-left",
          autoClose: 18050,
          hideProgressBar: false,
          closeOnClick: true,
        ...
        count = await coffeePortalContract.getTotalTips();

        console.log("Retrieved total tip count...", count.toNumber());

        setMessage("");
        setName("");

        toast.success("Tips Given!", {
          position: "top-left",
          autoClose: 5000,
          hideProgressBar: false,
          closeOnClick: true,
  ...
  const getAllTips = async () => {
    try {
      ...
      if (ethereum) {
         ...
         */
        const coffees = await coffeePortalContract.getAllTips();
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen py-2">
      <Head>
        <title>Yummy coffee mvp</title>
        <link rel="icon" href="/favicon.ico" />
      </Head>

      <main className="flex flex-col items-center justify-center w-full flex-1 px-20 text-center">
        <h1 className="text-6xl font-bold text-blue-600 mb-6">
          Tipping for crypto people
        </h1>
